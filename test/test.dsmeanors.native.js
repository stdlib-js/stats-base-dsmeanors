...
var dsmeanors = tryRequire( resolve( __dirname, './../lib/dsmeanors.native.js' ) );
var opts = {
	'skip': ( dsmeanors instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dsmeanors, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 3', opts, function test( t ) {
	t.strictEqual( dsmeanors.length, 3, 'has expected arity' );
	t.end();
});
...
tape( 'the functions throws an error if provided a first argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dsmeanors( value, new Float32Array( 10 ), 1 );
		};
	}
});
...
tape( 'the functions throws an error if provided a third argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dsmeanors( 10, new Float32Array( 10 ), value );
		};
	}
});
